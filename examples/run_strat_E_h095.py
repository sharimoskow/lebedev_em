...
GAMMA=1.0/2**0.5; Z_MIN,Z_MAX=-2.5,0.02; H=0.095   # <-- changed from 0.10
...
t0=time.time()
print(f"=== Strategy E  k={k}  H={H} ===",flush=True)
grid=symmetric_optimal_grid(H,300.,z_fd,GAMMA,k=k)
x_max=float(grid.x[-1])
print(f"Grid N_R={grid.N_R}  transverse domain ±{x_max:.2f} m  t={time.time()-t0:.1f}s",flush=True)
...
print(f"Solve done  t={time.time()-t0:.1f}s",flush=True)
...
if len(sc):
    ...
    print(f"No crossing  ratio={bxx[0]/bxz[0]:.3f} at z_min  t={time.time()-t0:.1f}s",flush=True)
    zc=None
