"""Strategy E with H=0.095, LGMRES solver.  Usage: python run_h095_lgmres.py [k]"""
import sys, os, time
...
LOGFILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'run_h095.log')
...
GAMMA=1.0/2**0.5; Z_MIN,Z_MAX=-2.5,0.02; H=0.095
...
log("Building diagonal preconditioner ...")
d = A_bc.diagonal()
d_inv = np.where(np.abs(d) > 1e-30, 1.0/d, 1.0)
M = spla.LinearOperator(A_bc.shape, matvec=lambda x: d_inv*x, dtype=complex)
log(f"  Diag precond built  total={time.time()-t0:.1f}s")

iters=[0]
def cb(r):
    iters[0]+=1
    if iters[0]%5==0:
        log(f"  iter {iters[0]}  res={np.linalg.norm(r):.3e}  t={time.time()-t0:.1f}s")

log("Solving LGMRES rtol=1e-8 ...")
t2=time.time()
E,info=spla.lgmres(A_bc,b_bc,M=M,rtol=1e-8,atol=0,maxiter=200,
                   inner_m=30,outer_k=10,callback=cb)
log(f"LGMRES info={info}  iters={iters[0]}  t={time.time()-t2:.1f}s  total={time.time()-t0:.1f}s")
...
if len(sc):
    ...
    log(f"CROSSING at z={zc:.4f} m")
else:
    log(f"No crossing  ratio={bxx[0]/bxz[0]:.3f} at z_min")
    zc=None
out=os.path.join(os.path.dirname(os.path.abspath(__file__)),f'hmin0095_k{k}_E.npz')
np.savez(out,z_x=z_x[mx],bxx=bxx,z_z=z_x[mx],bxz=bxz,
         z_cross=np.array([zc if zc is not None else np.nan]))
log(f"Saved {out}  total={time.time()-t0:.1f}s")
