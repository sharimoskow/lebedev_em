"""
run_geometry_func.py  —  Validate from_geometry_func with GeometryStack
for the DDH03 geometry (k=4, H=0.10m).

Compares crossing z against:
  - hmin010_k4_E.npz           (strategy-E exact geometry)
  - hmin010_k4_sigma_multimat.npz  (from_sigma_func with multi-material SVD)

Uses LGMRES + diagonal preconditioner.
"""
import sys, os, time
...
# ── sigma_func: same as run_sigma_func_multimat ───────────────────────────────
...
# ── Geometry stack (the new API) ──────────────────────────────────────────────
geo = GeometryStack([
    CylindricalBoundary(radius=R_BORE),      # borehole wall   (innermost)
    CylindricalBoundary(radius=R_INV),       # invasion wall
    PlanarBoundary(n_hat=N_HAT, d=D_PLANE),  # 60° dipping layer
])
...
if len(sc):
    ...
    print(f'\nCROSSING (geometry_func): z={zc:.4f} m')
else:
    zc = None; print('\nNo crossing found')

np.savez(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'hmin010_k4_geometry_func.npz'),
         z_x=z_w, bxx=bxx, bxz=bxz, z_cross=np.array([zc if zc else np.nan]))

# ── Compare against references ────────────────────────────────────────────────
here = os.path.dirname(os.path.abspath(__file__))

ref_E = os.path.join(here, 'hmin010_k4_E.npz')
if os.path.exists(ref_E):
    ref = np.load(ref_E)
    d2 = ref['bxx']-ref['bxz']; sc2 = np.where(np.diff(np.sign(d2)))[0]
    if len(sc2):
        zi=sc2[0]; zc_E=ref['z_x'][zi]-d2[zi]/(d2[zi+1]-d2[zi])*(ref['z_x'][zi+1]-ref['z_x'][zi])
        print(f'Reference (strategy-E):       z={zc_E:.4f} m')
        if zc: print(f'  Δ geometry_func vs E:       {abs(zc-zc_E)*1000:.0f} mm')

ref_mm = os.path.join(here, 'hmin010_k4_sigma_multimat.npz')
if os.path.exists(ref_mm):
    ref = np.load(ref_mm)
    d3 = ref['bxx']-ref['bxz']; sc3 = np.where(np.diff(np.sign(d3)))[0]
    if len(sc3):
        zi=sc3[0]; zc_mm=ref['z_x'][zi]-d3[zi]/(d3[zi+1]-d3[zi])*(ref['z_x'][zi+1]-ref['z_x'][zi])
        print(f'Reference (sigma_multimat):   z={zc_mm:.4f} m')
        if zc: print(f'  Δ geometry_func vs multimat: {abs(zc-zc_mm)*1000:.0f} mm')

print(f'\nTotal t={time.time()-t0:.1f}s', flush=True)
